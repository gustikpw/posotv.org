...
$(document).ready(function(){
  ...
  table = $('#table').DataTable({
    ...
    "ajax": {
      // "url": "http://localhost/api.poso.tv/bagian",
...
function save()
{
  ...
  var url;
  if(save_method == 'add') {
    url = "<?php echo site_url('bagian/save_bagian')?>";
  } else {
    url = "<?php echo site_url('bagian/update_bagian')?>";
  }
...
function edits(id)
{
  ...
  $('#form')[0].reset(); // reset form on modals
  $.ajax({
      url : "<?php echo site_url('bagian/get_edit/')?>" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {
...
function deletes(id)
{
  ...
  {
    $.ajax({
      url : "<?php echo site_url('bagian/delete_bagian')?>/"+id,
      type: "POST",
      dataType: "JSON",
      success: function(data) {
